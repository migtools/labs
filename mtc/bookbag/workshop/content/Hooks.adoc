...
There are currently 4 phases within a migration where migration hooks can br run. One hook may be run per phase for a maximum total of 4 hooks per migration, and hooks in any phase may be run either on the source or destination cluster.
 ...
 * *PostRestore*: After all restore tasks are complete.
...
=== Hook execution
When an Ansible playbook is provided as part of a migration hook it will be mounted into the hook container as a ConfigMap. In either case the hook container will be run as a job on the desired cluster, using the execution ServiceAccount and Namespace specified. Jobs are designed to run to completion, even if the initial pod is evicted or killed, and may attempt to run several times up until it it either reaches the backoffLimit or successful completion. We use the default backoffLimit of 6 for hook jobs.

=== Exercise
In this exercise, we will be using Migration Hooks to address a MediaWiki configuration setting change (https://www.mediawiki.org/wiki/Manual:$wgServer[$wgServer]) that needs to be updated to the destination OCP 4 cluster post application migration.  Let's get started.

MediaWiki is running on your source OCP 3 cluster in the `mediawiki` namespace.  Let's launch the MTC UI and create a Migration Plan:
...
We are now presented with Migration Options. Click Next.

image:./screenshots/lab4/mtc-mig-plan-migoptions.png[MTC Mig Plan Options]

Here is where we add our Migration Hook.  Let's click the `Add Hook` button.
...
We want our hook to repair the MediaWiki application on the OCP4 cluster, so let's select `Target Cluster` for where the ansible should be run.  We will need to also specify a `service account` and `namespace` for running the Hook Container.  For this example, let's choose (as these already exist on the cluster and have the needed permissions):

* Service account name: *migration-controller*
* Service account namespace: *openshift-migration*


Lastly, we have to choose which phase to run the hook.  Let's select *PostRestore*.  That's it for the Hook definition, so click `Add hook`.
...
Let's take a closer look at the mighook resource that was created during creation of the Migration Plan.  On our OCP 4 cluster, let's run the following:

[source,subs="{markup-in-source}"]
--------------------------------------------------------------------------------
# Switch to openshift-migration project
$ oc project openshift-migration
Now using project "openshift-migration" on server ...

$ **oc get mighook mediawiki-repair-db9pb -o yaml**
...
As you can see, our hook name has had a token postpended to ensure uniqueness.  The playbook has been base-64 encoded, and the hook has been validated and is ready for use with our plan.  Let's also take a look at the mediawiki migration plan:
...
As you can see, our hook is properly associated:
...
[source,subs="{markup-in-source}"]
--------------------------------------------------------------------------------
$ **oc get routes -n mediawiki -o jsonpath='{.items.*.spec.host}{"\n"}'**
migration-openshift-migration.apps.cluster-{OCP4_GUID}.{OCP4_GUID}.{OCP4_DOMAIN}
--------------------------------------------------------------------------------

image:./screenshots/lab8/mediawiki.png[MediaWiki]

*Success!*  In the next lab, we will explore some debugging techniques for when migrations don't go quite as planned.
