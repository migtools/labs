...
First thing we want to do is add the source OCP cluster we wish to migrate the application from. From the `Clusters` tab, click the `Add cluster` button. Fill out the necessary information. +
We will need an Service Account Token in order for MTC to communicate with the destination cluster:
...
URL field is FQDN of your OCP3 Master.
The last field is `Route to Image Registry` and this is provided so that you could include a route to the cluster's image registry if you want to allow direct image migration.
...
If we configured the direct image location when the cluster was added, we can choose to use the Direct image migration. In our case it will be unavailable. The rest can be left as it is. Click Next.
