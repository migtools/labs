...
`AWS`, `GCP`, `Azure`,`OpenShift`, `KubeVirt` and `CSI`. For installation, 
...
|Features |*Features |Features define the configuration for the DPA to enable the Technology Preview features.
|===
...
|Location to store volume snapshots. For further details, see [here](config/bsl_and_vsl.md).

|bucket
|[*CloudStorageLocation](https://pkg.go.dev/github.com/openshift/oadp-operator/api/v1alpha1#CloudStorage)
|[Technology Preview] Automates creation of a bucket at some cloud storage providers for use as a backup storage location.  For further details, see [here](https://access.redhat.com/support/offerings/techpreview).
|===
...
|===
|Property |Type |Description
|velero |[*VeleroConfig](https://pkg.go.dev/github.com/openshift/oadp-operator/api/v1alpha1#VeleroConfig) |This defines the configuration for the Velero
server

|restic |[*resticConfig](https://pkg.go.dev/github.com/openshift/oadp-operator/api/v1alpha1#ResticConfig) |This defines the configuration for the Restic
server
|===
...
|defaultPlugins |[] string |The following types of default Velero plugins can be
installed: `AWS` , `GCP` , `Azure`, `OpenShift`, `CSI`, and `KubeVirt` . See
...
|===
|Property |Type |Description
|enable |*bool |If set to `true`, enables backup/restore using Restic. If set to `false`,
...
|tolerations |https://pkg.go.dev/k8s.io/api/core/v1#Toleration[[]Toleration] |Tolerations defines the list of tolerations to be applied
to Velero Deployment/Restic daemonset

|resourceAllocations
|https://pkg.go.dev/k8s.io/api/core/v1#ResourceRequirements[ResourceRequirements]
